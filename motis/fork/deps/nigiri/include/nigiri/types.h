...
// kora fork: dedicated slot for the FULL (2 h) Valhalla transfer table
// (transfer-point-optimization.md § Two-tier transfer table). It gets a
// slot of its own because every named slot above carries semantics
// somewhere: build_lb_graph drops routes without bikes_allowed /
// cars_allowed from the bike / car lower-bound graphs, and profile 2 is
// hardwired as the wheelchair flag in the raptor drivers. Parking the
// full table in the bike slot cost its lower-bound graph nearly all
// transit edges (few Swiss routes carry bikes_allowed), degrading lower
// bounds to walk-the-whole-way times — RAPTOR then pruned every
// faster-with-more-transfers journey and, under a finite maxTravelTime,
