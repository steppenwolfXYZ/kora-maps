...
struct raptor {
  ...
  raptor(
    ...
    // a footpath with delta > 0 writes its target label into a FUTURE
    // round; these marks remember which stops must enter the route scan
    // once their round comes (station_mark_ only lives one round).
    pending_marks_.resize(kMaxTransfers + 2U);
    for (auto& b : pending_marks_) {
  ...
  void next_start_time() {
    ...
    // kora fork: like the station marks, pending future-round marks do
    // not carry over into the next start time (their round_times values
    // persist as valid labels, exactly like upstream's rounds do).
    clear_pending_marks();
  }
  ...
  // kora fork: seed_level = kora_walk_delta(access walk) — the walk to
  // this start stop is priced by seeding the label `seed_level` rounds
  // up, so the first boarding (+1) totals the walk's point class.
  void add_start(location_idx_t const l,
                 unixtime_t const t,
                 unsigned const seed_level) {
    auto const v = (Vias != 0 && is_via_[0][to_idx(l)]) ? 1U : 0U;
    trace_upd(
        "adding start [fwd={}] {}: {}, v={}, lvl={} [current: best={}, "
        "round={} => best={}]\n",
        kFwd, loc{tt_, l}, t, v, seed_level, to_unix(best_[to_idx(l)][v]),
    ...
    if (seed_level == 0U) {
      ...
      // A boarding from this seed lands at seed_level + 1, which must
      // still fit the round arrays.
      if (seed_level > kMaxTransfers) {
        return;
      }
      // best_ is NOT set: it must keep meaning "best over levels <= the
      // level currently processed" or it would wrongly prune
      // fewer-point labels; the per-round merge in execute() absorbs
      // this entry when round seed_level starts.
      round_times_[seed_level][to_idx(l)][v] = get_best(
          unix_to_delta(base(), t), round_times_[seed_level][to_idx(l)][v]);
      mark_pending(seed_level, to_idx(l));
    }
  }
  ...
  void execute(unixtime_t const start_time,
    ...
    for (auto k = 1U; k != end_k; ++k) {
      // kora fork (kora_walk_points.h): two-phase handling of labels
      // written ahead of their round by weighted walks / seeds.
      //
      // Phase 1 — VET pending[k] now, while best_ still covers levels
      // <= k-1 only (row k is merged below, after this): any equal-or-
      // better time in best_ was achieved with fewer points and
      // dominates the level-k label — including the equal-time case
      // the user-facing collapsing is all about. Dominated labels can
      // only spawn dominated journeys; clearing their mark here is
      // what keeps the level smear from multiplying the route scans.
      // (After row k is in best_, the label would be indistinguishable
      // from its own copy, so equality could not be used — hence the
      // vet-before-merge ordering.)
      if (max_pending_level_ >= k && k <= kMaxTransfers + 1U) {
        auto& pend = pending_marks_[k];
        kora_pending_dropped_.clear();
        pend.for_each_set_bit([&](std::uint64_t const i) {
          for (auto v = 0U; v != Vias + 1; ++v) {
            auto const rt = round_times_[k][i][v];
            if (rt != kInvalid && !is_better_or_eq(best_[i][v], rt)) {
              return;  // at least one via slot survives — keep
            }
          }
          kora_pending_dropped_.push_back(i);
        });
        for (auto const i : kora_pending_dropped_) {
          pend.set(i, false);
        }
      }

      // Phase 2 — CONSUME pending[k-1] (vetted last round): its labels
      // become boardable this round, so merge the surviving marks into
      // the station marks the route collection reads.
      if (max_pending_level_ >= k - 1U) {
        auto const& pend = pending_marks_[k - 1U];
        for (auto b = std::size_t{0U}; b != pend.blocks_.size(); ++b) {
          state_.station_mark_.blocks_[b] |= pend.blocks_[b];
        }
        utl::fill(pending_marks_[k - 1U].blocks_, 0U);
        if (max_pending_level_ == k - 1U) {
          // consumed in round order, so everything below is empty too
          max_pending_level_ = 0U;
        }
      }
      ...
      if (!any_marked) {
        ...
        // kora fork: labels may still be waiting in future rounds
        // (weighted walks / seeds) — only stop when none are pending.
        if (max_pending_level_ < k) {
          break;
        }
        utl::fill(state_.station_mark_.blocks_, 0U);
        continue;
      ...
      if (!any_marked) {
        trace_print_state_after_round();
        // kora fork: see the identical check above — future rounds may
        // still hold pending labels.
        if (max_pending_level_ < k) {
          break;
        }
        utl::fill(state_.route_mark_.blocks_, 0U);
...
private:
    ...
    state_.prev_station_mark_.for_each_set_bit([&](std::uint64_t const i) {
      ...
      for (auto const& fp : fps) {
        ...
        for (auto v = 0U; v != Vias + 1; ++v) {
          ...
          auto const fp_dur = adjusted_transfer_time(transfer_time_settings_,
                                                     fp.duration().count());
          auto const fp_target_time =
              clamp(tmp_time + dir(fp_dur + stay.count()));

          // kora fork: walk-weighted transfer points — a footpath whose
          // walk class costs extra points writes its target into round
          // k + delta instead of k (kora_walk_points.h). best_ must not
          ...
          if (kd != 0U) {
            ...
            if (tk <= kMaxTransfers + 1U &&
                (bounds_last_k_ == 0U || tk <= bounds_last_k_) &&
                is_better(fp_target_time, best_[target][target_v]) &&
                is_better(fp_target_time,
                          round_times_[tk][target][target_v]) &&
                is_better_loose(fp_target_time, time_at_dest_[tk]) &&
                lb_reachable(target) &&
                is_better_loose(fp_target_time + dir(get_lb(target)),
                                time_at_dest_[tk]) &&
                (bounds_last_k_ == 0U ||
                 within_bounds(tk, target, fp_target_time, target_v))) {
              ++stats_.n_earliest_arrival_updated_by_footpath_;
              round_times_[tk][target][target_v] = fp_target_time;
              mark_pending(tk, target);
              if (target_v == Vias && is_dest_[target]) {
                update_time_at_dest(tk, fp_target_time);
              }
            }
            continue;
          }
  ...
  void update_intermodal_footpaths(unsigned const k) {
    ...
    state_.prev_station_mark_.for_each_set_bit([&](auto const i) {
      ...
      if (dist_to_end_[i] != std::numeric_limits<std::uint16_t>::max()) {
        ...
        // kora fork: walk-weighted transfer points — the egress walk
        // prices the arrival dist_to_end_ minutes long into round
        // k + delta (kora_walk_points.h). kIntermodalTarget never
        ...
        if constexpr (Vias != 0U) {
          ...
          if (tmp_[i][v] != kInvalid && is_via_[v][i]) {
            ...
            if (kora_end_ok && is_better(end_time, time_at_dest_[ek]) &&
                is_better(end_time,
                          round_times_[ek][kIntermodalTarget][Vias])) {
              round_times_[ek][kIntermodalTarget][Vias] = end_time;
              if (kora_end_delta == 0U) {
                best_[kIntermodalTarget][Vias] = end_time;
              }
              update_time_at_dest(ek, end_time);
              trace_upd(" -> update\n");
            } else {
              trace_upd(" -> no update\n");
            }
        ...
        if (kora_end_ok && is_better(end_time, time_at_dest_[ek]) &&
            is_better(end_time, round_times_[ek][kIntermodalTarget][Vias])) {
          round_times_[ek][kIntermodalTarget][Vias] = end_time;
          if (kora_end_delta == 0U) {
            best_[kIntermodalTarget][Vias] = end_time;
          }
      ...
      if (auto const it = td_dist_to_end_.find(l); it != end(td_dist_to_end_)) {
        ...
        if (fp.has_value()) {
          ...
          if (td_ek <= kMaxTransfers + 1U &&
              is_better(end_time, time_at_dest_[td_ek]) &&
              is_better(end_time, best_[kIntermodalTarget][Vias]) &&
              is_better(end_time,
                        round_times_[td_ek][kIntermodalTarget][Vias])) {
            round_times_[td_ek][kIntermodalTarget][Vias] = end_time;
            if (td_kd == 0U) {
              best_[kIntermodalTarget][Vias] = end_time;
            }
            update_time_at_dest(td_ek, end_time);
  ...
  // kora fork: walk-weighted transfer points helpers.
  void mark_pending(unsigned const level, std::size_t const l) {
    pending_marks_[level].set(l, true);
    max_pending_level_ = std::max(max_pending_level_, level);
  }

  void clear_pending_marks() {
    if (max_pending_level_ != 0U) {
      for (auto& b : pending_marks_) {
  ...
  // kora fork: per-future-round station marks for labels written ahead
  // of the current round by walk-weighted footpaths / seeds.
  std::vector<bitvec> pending_marks_;
  unsigned max_pending_level_{0U};
  // scratch for the vet phase (reused to avoid per-round allocation)
  std::vector<std::uint64_t> kora_pending_dropped_;
  day_idx_t base_;
  raptor_stats stats_;
