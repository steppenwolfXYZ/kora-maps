...
struct journey {
  ...
  // kora fork: when set, this journey is an ε-alternate
  // (near-optimal-endpoint-alternatives.md): reconstruction must route
  // the intermodal destination leg through exactly this stop, anchored
  // at dest_time_ instead of the optimal round_times entry.
  location_idx_t kora_alt_egress_{location_idx_t::invalid()};
};
