...
struct search_state {
  ...
  // reconstructed journeys within q.kora_alt_epsilon_ of a Pareto
  // point, differing in the egress/access stop. Kept outside results_
  // because they are dominated by definition and the pareto_set would
  // reject them.
  std::vector<journey> alternatives_;
};
...
struct search {
  ...
  Algo init(clasz_mask_t const allowed_claszes,
    ...
    // kora fork: keep the algo's base day — collect_alternatives needs
    // it for delta_t ↔ unixtime conversions that must match Algo's own.
    base_ = day_idx_t{
  ...
  routing_result execute() {
    ...
    // kora fork: ε-alternates pass the same validity filters as the
    // primary journeys (interval containment, direct/travel-time caps),
    // then quay-blind dedupe against the primaries and each other.
    utl::erase_if(state_.alternatives_, [&](journey const& j) {
      return !j.is_reconstructed_ ||
             (is_pretrip() && (!search_interval_.contains(j.start_time_) ||
                               j.travel_time() >= fastest_direct_ ||
                               j.travel_time() > q_.max_travel_time_));
    });
...
private:
  ...
  void search_interval() {
    ...
    utl::equal_ranges_linear(
        ...
        [&](auto&& from_it, auto&& to_it) {
          ...
          collect_alternatives(start_time);
  ...
  // kora fork: ε-alternates (near-optimal-endpoint-alternatives.md).
  // RAPTOR keeps a per-stop optimum, but only the best (stop arrival +
  // egress offset) combination is ever read out per Pareto point — an
  // equal-or-slightly-worse journey over a different egress stop is
  // fully computed and silently discarded. This hook runs inside the
  // per-start-time loop, while the algo state for this start time is
  // still live, and reconstructs those near-optimal endpoint variants
  // into state_.alternatives_. The search itself is untouched.
  //
  // Candidate arrival via stop s: round_times[k][s] carries the stop's
  // change-time buffer (update_transfers), while the intermodal target
  // was fed from the raw transport arrival (tmp_) — so the buffer is
  // subtracted again. round_times[k][s] written by a transfer footpath
  // instead yields an infeasible candidate; reconstruction validates
  // every candidate against the algo state and infeasible ones are
  // dropped, so a wrong guess costs one failed reconstruction, never a
  // wrong journey.
  void collect_alternatives(unixtime_t const start_time) {
    if constexpr (requires(algo_state_t& s) {
                    s.template get_round_times<0>();
                  }) {
      if (q_.kora_alt_epsilon_ == duration_t{0} || q_.kora_alt_max_ == 0U ||
          q_.dest_match_mode_ != location_match_mode::kIntermodal ||
          !q_.via_stops_.empty()) {
        return;
      }
      auto const base =
          tt_.internal_interval_days().from_ + to_idx(base_) * date::days{1};
      auto const round_times = algo_state_.template get_round_times<0>();
      // Fingerprints of everything already known — primaries and earlier
      // groups' alternates — so extraction only spends acceptance slots
      // on genuinely new vehicle combinations.
      auto seen = std::vector<kora_fingerprint_t>{};
      for (auto const& r : state_.results_) {
        if (r.is_reconstructed_ && !r.error_) {
          seen.emplace_back(kora_transit_fingerprint(tt_, r));
        }
      }
      for (auto const& r : state_.alternatives_) {
        seen.emplace_back(kora_transit_fingerprint(tt_, r));
      }
      for (auto const& j : state_.results_) {
        if (!j.is_reconstructed_ || j.error_ || j.start_time_ != start_time ||
            j.kora_alt_egress_ != location_idx_t::invalid()) {
          continue;
        }
        kora_collect_endpoint_alternatives<SearchDir>(
            tt_, rtt_, q_, j, round_times, base, seen,
            /*primary_in_seen=*/true,
            [&](journey& a) { algo_.reconstruct(q_, a); },
            state_.alternatives_);
      }
    }
  }
  ...
  // kora fork: algo state ref + base day for collect_alternatives.
  // Both declared before algo_ — init() assigns base_ while algo_ is
  // being constructed.
  algo_state_t& algo_state_;
  day_idx_t base_{};
  Algo algo_;
