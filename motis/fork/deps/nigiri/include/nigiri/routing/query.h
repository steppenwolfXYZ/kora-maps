...
struct query {
  ...
  // For each Pareto-optimal journey of an intermodal-destination query,
  // egress (fwd) / access (bwd) stops whose door-to-door time lands
  // within this slack of the optimum are read out of the finished
  // RAPTOR state as additional journeys. 0 disables (upstream
  // behavior). kora_alt_max_ caps the alternates per Pareto point.
  duration_t kora_alt_epsilon_{0};
  std::uint8_t kora_alt_max_{0U};
};
