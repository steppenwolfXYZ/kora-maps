...
void reconstruct_journey_with_vias(timetable const& tt,
  ...
  auto const is_transport_active = [&](transport_idx_t const t,
    ...
    if (rtt != nullptr) {
      ...
      return tt.is_transport_active(t, day);
    }
  };
  ...
  auto const find_dest_leg = [&](unsigned const k, location_idx_t const l,
    ...
    if (dest_offset.duration_ >= footpath::kMaxDuration) {
      ...
      // one that should be reconstructed
      return std::nullopt;
    }
    ...
    // alternate's arrival is by definition not the optimum recorded
    // there.
    auto const curr_time =
        j.kora_alt_egress_ == location_idx_t::invalid()
            ? round_times[k][to_idx(l)][v]
            : unix_to_delta(base, j.dest_time_);
    for_each_meta(
  ...
  auto const get_legs =
    ...
    if (q.dest_match_mode_ == location_match_mode::kIntermodal &&
      ...
      for (auto const& dest_offset : q.destination_) {
        // kora fork: an ε-alternate reconstructs through exactly its
        // forced egress/access stop — every other offset would resolve
        // back to a different stop's leg at the synthesized dest_time_.
        if (j.kora_alt_egress_ != location_idx_t::invalid() &&
            dest_offset.target_ != j.kora_alt_egress_) {
          continue;
        }
        // kora fork: ε-alternates anchor at an UPPER BOUND of the true
        // arrival, so the exact-minute transport matching of the plain
        // path can never hit. The td_footpath matching mode accepts any
        // transport arriving at or before the anchor — the caller snaps
        // the leg times to the transport actually found afterwards.
        auto const ret = find_dest_leg(
            k, l, dest_offset,
            /*td_footpath=*/j.kora_alt_egress_ != location_idx_t::invalid());
        if (ret.has_value()) {
          return std::move(*ret);
        }
      }
      ...
      for (auto const& [from, td] : q.td_dest_) {
        // kora fork: no td fallback for ε-alternates — their candidate
        // time was derived from a plain offset, so a td leg resolving
        // at the synthesized time would be a different stop's journey.
        if (j.kora_alt_egress_ != location_idx_t::invalid()) {
          break;
        }
...
void reconstruct_journey(timetable const& tt,
                         ...
                         date::sys_days const base,
                         day_idx_t const base_day_idx) {
  // kora fork: kMaxVias raised to 3 (via-stops.md) — the case below was
  // added to match.
