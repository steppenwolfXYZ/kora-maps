// Kora Maps fork of nigiri/src/routing/raptor/optimize_footpaths.cc
// (pinned nigiri commit — see MOTIS_REF in the fork Dockerfile; nigiri is
// pinned transitively via MOTIS's .pkg).
//
// Patch (marked "kora fork:"): optimize_transfers() additionally considers
// re-boarding at the very same stop the previous leg alights at. Upstream
// only scans footpath pairs, and neither GTFS transfers.txt nor the
// imported Valhalla matrix carries self-entries, so a zero-walk
// same-platform transfer was invisible to the optimizer and a longer
// street walk would win. Feasibility uses the per-stop change time — the
// same rule the RAPTOR search applies for same-stop transfers. Also fixes
// the incumbent penalty to score the actual transfer stops instead of the
// journey endpoints. See transfer-point-optimization.md.
...
void optimize_transfers(timetable const& tt,
  ...
  for (auto i = 0U; i + 2 < j.legs_.size(); ++i) {
    ...
    // kora fork: score the incumbent at its actual transfer stops
    // (upstream passed the journey's endpoints, denying the incumbent the
    // same-station bonus and biasing toward sliding).
    auto penalty_best = get_penalty(tt, get<footpath>(leg_footpath.uses_).duration(),
                                    leg_to.dep_time_ - leg_footpath.arr_time_,
                                    leg_footpath.from_, leg_footpath.to_);
    for (auto stp_from : fr_from) {
      ...
      for (auto stp_to : fr_to) {
        ...
        // kora fork: same-stop re-boarding candidate. No footpath table
        // carries self-entries, so this case needs the per-stop change
        // time (mirroring the search's same-stop transfer rule).
        if (stp_from.get_location_idx() == stp_to.get_location_idx()) {
          auto const l = stp_from.get_location_idx();
          auto const fp_dur = adjusted_transfer_time(
              q.transfer_time_settings_,
              duration_t{tt.locations_.transfer_time_[l]});
          auto const arr = stp_from.time(event_type::kArr);
          auto const dep = stp_to.time(event_type::kDep);
          auto const arr_fp = arr + fp_dur;
          if (arr_fp <= dep) {
            auto const penalty = get_penalty(tt, fp_dur, dep - arr_fp, l, l);
            if (penalty < penalty_best) {
              ...
              leg_footpath.from_ = l;
              leg_footpath.to_ = l;
              leg_footpath.dep_time_ = arr;
              leg_footpath.arr_time_ = arr_fp;
              leg_footpath.uses_ = footpath{l, fp_dur};

              penalty_best = penalty;
            }
          }
          continue;  // a footpath can never target the stop itself
        }
