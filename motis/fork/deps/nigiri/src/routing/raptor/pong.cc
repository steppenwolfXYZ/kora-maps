...
routing_result pong(timetable const& tt,
      ...
      .search_stats_ =
          {.lb_time_ = static_cast<std::uint64_t>(
               std::chrono::duration_cast<std::chrono::milliseconds>(lb_time)
                   .count())},
      .algo_stats_ = {}};
  // kora fork: fingerprints of alternates accepted so far, across all
  // cursors — extraction dedupes against it inline; the primaries'
  // duplicates fall to the final kora_dedupe_alternatives.
  auto kora_alt_seen = std::vector<kora_fingerprint_t>{};
  // The loop re-discovers the same Pareto journeys at every cursor
  // between the window start and their departure. Extraction is per
  // Pareto point, not per rediscovery — running it each cursor
  // multiplied the whole extraction cost ~5-8x for zero extra results.
  auto kora_extracted = std::vector<std::pair<unixtime_t, std::uint8_t>>{};
  auto start_time =
  ...
  while ((is_better(start_time, end_time) ||
    ...
    // The forward ping state holds the egress side (access side for
    // arrive-by) and its journeys already carry the final arrival time
    // and transfer count — only the departure gets tightened by the
    // pong pass later. Extract endpoint alternates HERE, while the
    // shared raptor state still holds the ping round times
    // (pong.reset_arrivals() below wipes them), reconstructing against
    // the ping algo with the still-unflipped query. The primary's own
    // egress stop is unknown at this point (ping journeys are never
    // reconstructed); its duplicate alternate is removed by the
    // fingerprint dedupe at the end.
    if constexpr (!kGpu) {
      if (q.kora_alt_max_ != 0U && q.kora_alt_epsilon_ != duration_t{0} &&
          q.via_stops_.empty()) {
        auto const alt_round_times = r_state.template get_round_times<0>();
        auto const alt_base =
            tt.internal_interval_days().from_ + to_idx(base_day) * date::days{1};
        for (auto const& ping_j : ping_results) {
          if (ping_j.travel_time() >= fastest_direct ||
              ping_j.travel_time() >= q.max_travel_time_) {
            continue;
          }
          auto const key = std::pair{ping_j.dest_time_, ping_j.transfers_};
          if (std::find(begin(kora_extracted), end(kora_extracted), key) !=
              end(kora_extracted)) {
            continue;
          }
          kora_extracted.emplace_back(key);
          kora_collect_endpoint_alternatives<SearchDir>(
              tt, rtt, q, ping_j, alt_round_times, alt_base, kora_alt_seen,
              /*primary_in_seen=*/false,
              [&](journey& a) { ping.reconstruct(q, a); },
  ...
  // kora fork: ε-alternates — same validity caps as the primaries, then
  // quay-blind dedupe against them and each other. No start/dest or
  // special-station swap: the alternates come from the ping pass, which
  // ran in the original query orientation (the swaps above exist only
  // because the primaries were reconstructed by the flipped pong pass).
  utl::erase_if(s_state.alternatives_, [&](journey const& a) {
