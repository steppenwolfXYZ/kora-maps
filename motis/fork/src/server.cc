...
int server(data d, config const& c, std::string_view const motis_version) {
  // kora fork: walking is Valhalla-only in this build (see
  // valhalla-pedestrian-router.md) — refuse to serve without it rather
  // than degrade. Probed here and not at import: import consumes the
  // precomputed footpath matrix CSV, never live Valhalla.
  kora_valhalla::ensure_reachable_or_abort();
