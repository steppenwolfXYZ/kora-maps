...
std::vector<n::routing::offset> get_offsets(
  ...
  auto const handle_mode = [&](api::ModeEnum const m) {
    ...
    if (profile == osr::search_profile::kFoot) {
      // kora fork: Valhalla is the sole walking authority — the WALK
      // offsets that seed RAPTOR's start/destination stops come from a
      // Valhalla one-to-many matrix call, not from OSR (see
      // valhalla-pedestrian-router.md). This is what makes RAPTOR pick
      // boarding stops based on real walking times.
      //
      // Radius capped at 20 km (~4 h at 5.1 km/h): the app's wide
      // escalation can request an 8 h budget, but a 40 km candidate
      // radius would mean tens of thousands of matrix targets per
      // query. Targets additionally capped at Valhalla's per-request
      // location limit, nearest-first — one request, one graph
      // expansion.
      constexpr auto const kMaxFootRadius = 20'000.0;
      constexpr auto const kMaxFootTargets = std::size_t{2400};

      auto near_stops = get_stops_with_traffic(
          *r.tt_, rtt, *r.loc_tree_, pos, std::min(max_dist, kMaxFootRadius));
      if (near_stops.size() > kMaxFootTargets) {
        std::sort(begin(near_stops), end(near_stops),
                  [&](auto const a, auto const b) {
                    return geo::distance(pos.pos_,
                                         r.tt_->locations_.coordinates_[a]) <
                           geo::distance(pos.pos_,
                                         r.tt_->locations_.coordinates_[b]);
                  });
        near_stops.resize(kMaxFootTargets);
      }

      auto const stop_coords =
...
std::vector<n::routing::offset> routing::get_offsets(
  ...
  auto const do_get_offsets = [&](osr::location const pos,
                                  std::vector<api::ModeEnum> const& m) {
    return ::motis::ep::get_offsets(*this, rtt, pos, dir, elevations_, m,
  ...
  return std::visit(
      utl::overloaded{
          [&](tt_location const l) {
            ...
            // kora fork: WALK offsets for station endpoints come from
            // the imported Valhalla footpath matrix (the transfer
            // table) instead of a live Valhalla call — the matrix
            // already holds this station's walking times to every stop
            // within max_footpath_length, including nearby stations
            // RAPTOR may prefer to board at. Zero HTTP per query.
            // Limits: reach is capped at max_footpath_length (2 h) —
            ...
            if (wants_walk) {
              ...
              offsets.reserve(best.size());
              for (auto const& [target, duration] : best) {
                offsets.emplace_back(target, duration,
                                     static_cast<n::transport_mode_id_t>(
                                         osr::search_profile::kFoot));
              }
            }

            auto other_modes = modes;
            std::erase(other_modes, api::ModeEnum::WALK);
            if (!other_modes.empty()) {
              auto const coord_offsets = do_get_offsets(
                  stop_to_osr_location(*this, l.l_), other_modes);
              offsets.insert(end(offsets), begin(coord_offsets),
                             end(coord_offsets));
            }
          ...
          [&](osr::location const& pos) { return do_get_offsets(pos, modes); }},
      p);
}
