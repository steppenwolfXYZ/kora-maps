// kora fork: patched copy of upstream include/motis/osr/parameters.h
// (MOTIS_REF pin). One addition — `kora_walk_factor_`, the query-scoped
// scaling the Valhalla walking surfaces (get_offsets WALK branch,
// street_routing kFoot intercept) apply to their durations. Valhalla is
// always queried at the base kWalkSpeedKmh (kora_valhalla.h) so the
// coordinate-keyed caches stay shared across walking speeds; the factor
// rescales the returned durations to the client-requested speed
// (`pedestrianSpeed` API param). 1.0 = base speed = upstream behavior.
...
struct osr_parameters {
  ...
  // kora fork: multiplier applied to Valhalla walking durations
  // (kWalkSpeedKmh-based) to express the requested walking speed:
  // factor = kWalkSpeedKmh / requested_kmh. Set only by the plan
