...
`scripts/update_map.sh` chains the data-refresh machine's whole routine — `rebuild_transit.sh --start 1 --force-gtfs --force-atlas` (plus `--force-osm` with `--osm`), a Valhalla tile wipe when the OSM extract is newer than the tiles, `setup_routing.sh --force-matrix --force-import`, then the three deploys (`deploy_motis.sh --data-only`: indexes only, never this machine's amd64 image) and a production smoke test. It aborts at the first failing stage, so a broken local import never reaches the server. The app deploy stays separate (git push from the dev Mac).
