...
The MOTIS binary is the Kora fork (`motis/fork/`, image tag `koramaps/motis:footpath-matrix`) — Valhalla is the sole walking authority end to end (see `valhalla-pedestrian-router.md` and `motis/fork/README.md`): the import-time transfer table loads the precomputed Valhalla matrix (`KORA_FOOTPATH_MATRIX_PATH` → `/data/data/valhalla_footpath_matrix.csv`, abort if missing), floored per quay pair by the feed's own minimum transfer times read from `transfers.txt` (`KORA_GTFS_TRANSFERS_PATH` → `/data/gtfs/transfers.txt`, abort if missing — see `transfer-point-optimization.md` § Minimum transfer time), and at query time the fork calls Valhalla live for WALK offsets (RAPTOR boarding-stop selection) and WALK legs (`KORA_VALHALLA_URL`, default `http://kora-valhalla:8002`). No OSR walking fallback anywhere; the fork's server exits at startup while Valhalla is unreachable and docker's restart policy retries until it is. MOTIS and Valhalla containers share the external docker network `koramaps` (one-time, per machine: `docker network create koramaps`). Build the image locally with `./scripts/routing/setup_routing.sh --steps 2`: it first builds (once per machine and per `MOTIS_REF`) the tagged baseline image `koramaps/motis-baseline:<sha>` from `motis/fork/Dockerfile.baseline` — toolchain, upstream clone, all ~30 dependency clones, full compile — then the fork on top of it. A tagged image is never garbage-collected, unlike the cached layer it replaced (BuildKit's GC once evicted that layer, and every retry re-cloned the 30 dependency repos until GitHub rate-limited the machine). The upstream MOTIS commit is pinned via `MOTIS_REF` in the Dockerfile — bump procedure in `motis/fork/README.md`.

**The browser never talks to MOTIS** (`server-side-transit-planning.md`). The transit search is the app's own endpoint `GET /api/plan` (`src/routes/api/plan/+server.ts`): the node app runs the whole search cascade — narrow / wide walking budgets, the time-advance hops, dominance pruning — against MOTIS over loopback (`MOTIS_INTERNAL_URL`, `$env/dynamic/private`, default `http://localhost:8080`, prod `http://127.0.0.1:8080` in the `ENV_VARS` secret; the same variable share re-verification already used) and returns the final list, so one user action is one HTTP request. There is no `PUBLIC_MOTIS_URL` any more and **no nginx `location /routing/`** — MOTIS stays loopback-bound and reachable only from the app process and the `koramaps` docker network (remove the location from the site file when deploying this change; the stats page counts `/api/plan` lines). Docker (not pm2) supervises the container (`restart: unless-stopped` + enabled `docker.service`).
...
Same software / data split as the MOTIS deploy. Default ships the **software**: the locally-built Kora fork image `koramaps/valhalla:bicycle-costing` (`valhalla/fork/`, built by `setup_routing.sh` step 2b on top of the tagged baseline image `koramaps/valhalla-baseline:<VALHALLA_REF>` from `valhalla/fork/Dockerfile.baseline`, same once-per-machine-and-pin rule as the MOTIS baseline) via `docker save | ssh docker load` plus `valhalla/docker-compose.prod.yml` → `valhalla/` on the server, then `up -d` recreates the container. `valhalla/data/` (Valhalla tiles + Mapterhorn elevation + admin polygons, ~500-800 MB depending on the OSM extract) ships only on explicit request: `--with-data` adds it (dev-Mac exception), `--data-only` ships data + compose without the image (the data machine's mode, used by `update_map.sh` — its amd64 image must never reach the arm64 server; the script refuses a non-arm64 image outright). When data ships the script stops the container before rsync because Valhalla memory-maps its tiles. Prod compose is serve-only (`use_tiles_ignore_pbf=True`, no PBF mounted, no elevation download), loopback-bound (`127.0.0.1:8002`), `mem_limit: 1g`, capped json-file logs.
...
- MOTIS diagnostics: `docker logs kora-motis --tail 50`, `docker stats kora-motis --no-stream`. External smoke test (through the app's endpoint — MOTIS itself is not exposed): `curl 'https://koramaps.app/api/plan?fromPlace=47.378,8.540&toPlace=47.424,8.508&arriveBy=false&time=2026-09-09T12:00:00Z'` should return JSON with an `itineraries` array. On the server, MOTIS directly: `curl 'http://127.0.0.1:8080/api/v1/plan?fromPlace=47.378,8.540&toPlace=47.424,8.508&arriveBy=false&numItineraries=1&directModes=WALK'`.
- MOTIS crash-looping with `[VERIFY FAIL] osr_footpath: no existing version found … hashes: {}`: `motis/data/meta/osr_footpath.json` is missing. `setup_routing.sh` step 2 deletes it after every image rebuild so that step 7 redoes the transfer table — if step 7 never follows, the running server keeps serving (the stamp is read only at startup) and the next restart, typically a reboot, fails. Fix: re-import (`setup_routing.sh --steps 7`) or, when the indexes came from Kranich unchanged, copy its `motis/data/meta/osr_footpath.json` back. After a by-hand `docker build` of the fork, `touch motis/.image_build_stamp` so a later `setup_routing.sh` run doesn't rebuild and delete the stamp again.
- `[VERIFY FAIL] … reconstruction failed` lines in the MOTIS log are real: ε-alternate candidates that reconstruction rejects throw silently (see `motis/fork/README.md`), so what still prints is a primary journey failing to reconstruct.
- Transit queries with a coordinate endpoint that take 10+ s cold (fast on repeat) in one area: almost certainly a quay on a walking island in the Valhalla graph — the fork's offset matrix must exhaust the graph before it can skip an unreachable target. Run `python3 scripts/routing/find_isolated_quays.py` against the current matrix; it must list nothing. Fix in the walk-network builder / OSM, never by tuning the client cascade (`station-walk-network.md` § Island check).
