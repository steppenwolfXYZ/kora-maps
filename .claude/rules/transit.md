...
- `mountain_agency_ids` — exact GTFS `agency_id` whitelist. Each listed agency's `route_type=2` (rail) trips are rebucketed to `mountain` at `load_trips` time, which feeds every downstream bucket-keyed exemption (active-days filter, freq gate, yellow mode). Criterion: "agency is outside the GA-Geltungsbereich"; Rigi Bahnen is a manual exception. Always agency_id, never name substrings — the substring approach previously matched both BOB main (valley) and BOB-spb (Schynige Platte) and broke the Interlaken–Lauterbrunnen line.
...
Mountain classification comes from GTFS route_type (5/6/7) plus the `mountain_agency_ids` whitelist (rebuckets listed agencies' `route_type=2` rail to `mountain` at `load_trips`). Rack railways outside that whitelist stay in `train`. `deduplicate_mountain()` collapses overlapping **aerial** features (route_type 5/6) that share a ref — funiculars (7), rebucketed rail, and other modes are not collapsed because a shared bbox typically indicates two different branches off a common stem rather than a duplicate.
