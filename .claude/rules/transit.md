...
**Exemptions** are scoped by `mountain_origin`, not the bucket alone, and cover three rules that all apply to the same set: per-direction split, freq-score gate (`f_weighted > worst_freq`), and active-days gate (`min_active_days`). `ferry`, `aerial`, and `funicular` are fully exempt from all three — their two directions collapse into one feature, and neither gate gates them. `rack` and `rebucketed_rail` are treated like normal `train`: per-direction split applies, both gates apply. The rare-group filter still treats the full mountain bucket as exempt. The active-days gate threshold is per-bucket via `min_active_days_by_bucket` in `scripts/transit/config.yaml` — currently `mountain: 90` so summer-only rack railways (Schynige Platte, Brienzer Rothorn) still draw; everything else falls back to the `min_active_days` default of 150.
