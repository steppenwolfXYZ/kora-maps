...
**Stack:** SvelteKit frontend (`src/routes/`), MapLibre GL JS (`src/routes/Map.svelte`), style generated from `scripts/config.yaml` → `scripts/generate_style.py` → `static/map-assets/style.json`. **Routing** via a local MOTIS v2 instance in `motis/` (docker-compose + config) — see `transit-routing.md` for the engine + panel and `route-display.md` for the on-map rendering. `scripts/preprocess_osm_for_motis.py` rewrites the country OSM PBF once before import to add `foot=yes` to CH `access=agricultural`/`forestry` ways so alp / forest roads route for pedestrians. `scripts/preprocess_gtfs_for_motis.py` builds `data/gtfs_motis/` as a MOTIS-only sidecar with a platform-snapped `stops.txt` (rest of the GTFS hardlinked from `data/gtfs_routed/`), keeping the pipeline's map artifacts untouched — see `transit-routing.md` § Backend.
