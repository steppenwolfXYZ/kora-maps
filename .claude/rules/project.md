...
**Stack:** SvelteKit frontend (`src/routes/`), MapLibre GL JS (`src/routes/Map.svelte`), style generated from `scripts/config.yaml` → `scripts/generate_style.py` → `static/map-assets/style.json`. **Routing** via a forked local MOTIS v2 instance in `motis/` (fork sources in `motis/fork/`) plus a Valhalla pedestrian router in `valhalla/` — Valhalla is the sole walking authority (transfer table from a precomputed matrix at import, live calls for query-time walks; no OSR walking fallback) — see `valhalla-pedestrian-router.md`, `transit-routing.md` for the engine + panel, and `route-display.md` for the on-map rendering. `scripts/preprocess_osm_for_motis.py` adds `foot=yes` to CH `access=agricultural`/`forestry` ways so alp / forest roads route for pedestrians (CH-only PBF for MOTIS; `--valhalla` preset for the wide-bbox Valhalla input). `scripts/build_station_walk_network.py` synthesises the routable station interior Valhalla cannot derive itself — a walk line down every mapped platform, welded into the pedestrian graph only across compatible levels, plus lift hubs — and anchors every GTFS quay onto it; see `station-walk-network.md`. `scripts/preprocess_gtfs_for_motis.py` builds `data/gtfs_motis/` as a MOTIS-only sidecar with a platform-anchored `stops.txt` (rest of the GTFS hardlinked from `data/gtfs_routed/`), keeping the pipeline's map artifacts untouched — see `transit-routing.md` § Backend. `scripts/build_valhalla_footpath_matrix.py` precomputes the stop-to-stop walking matrix the fork imports (`matrix_build_remote.md` for running it on a beefier machine).
