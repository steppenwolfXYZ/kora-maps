...
- **Degenerate-bar drop.** A candidate sweep position whose drawn span (the perpendicular extent of its scoring dots along the bar's transverse axis) is less than `DEGENERATE_BAR_MIN_SPAN_M` (1 m) is rejected. Such a "bar" renders as a dot, not a bar — the would-be stabbed members are handed to the leftover-fill instead. This catches the case where two scoring platforms have distinct snapped coords (so the two-distinct-positions gate passes) but their bar-intersection points sit only a few decimetres apart (e.g. two lines sharing one physical platform whose GTFS coords differ by 20-30 cm).
...
For tram / bus / regional_bus only, an additional rule drops arrival entries that the rule above did not pair with a departure. The typical case is a layover bay where the vehicle drops passengers ~100 m from the real terminus before moving to the actual departure stop: GTFS encodes the layover and the departure as separate `stop_id`s, so the `stop_id`-keyed pairing above cannot match them. Such an arrival entry is **treated as nonexistent by the pill-construction algorithm** and omitted from production stop dots, pill clusters, debug platform extents and debug dots — the same scope of omission as the rule above. The underlying polyline is unchanged: the line still extends visually to the layover bay. Loop trips are technically in scope; in Swiss feeds this rarely matters because circular routes are split into two directional segments.
