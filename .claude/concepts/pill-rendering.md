...
- **Single-group cluster with > 1 tied option:** enumerate the group's tied options. For each option, place the bar's scoring + covered dots, run the leftover baseline if applicable, and measure **pill + 0.5 × connector length** across the whole cluster (same metric as the Candidate A/B comparison). Pick the shortest. This runs regardless of leftover count — even a single leftover (a singleton group that doesn't form a sub-cluster of its own) still participates in the cluster's NN-path / MST connector, so the connector's length depends on where the bar sits.
