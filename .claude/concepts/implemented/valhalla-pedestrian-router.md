...
- **Every quay must be reachable in the pedestrian graph.** The one-to-many matrix call that seeds RAPTOR's boarding stops for a coordinate endpoint can only report an unreachable target after exhausting the whole graph within the walking limit, so a single island quay inside a query's candidate radius costs seconds on every cold query nearby — the performance symptom is indistinguishable from a slow engine. Islands are prevented in the station walk network build and audited with `find_isolated_quays.py` after each matrix build (`station-walk-network.md` § Island check).
