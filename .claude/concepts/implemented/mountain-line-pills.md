...
- **`aerial`** — **fixed dot** (see below). Aerial cable-car / gondola / elevator stations have no platform geometry and zero atlas length coverage. Each aerial stop locks its dot to the snapped GTFS coord and joins the pill pipeline with no extent freedom. Aerial features are additionally **exempt from terminus dedup** — see "Terminus-dedup exemption for direction-collapsed modes" below.

### Terminus-dedup exemption for direction-collapsed modes

Modes whose opposite directions are collapsed into one emitted feature upstream — **ferry**, **aerial mountain**, and **funicular mountain** (the modes carrying the `("*", "*")` direction_key sentinel per `.claude/rules/transit.md` § "Mountain and ferry") — are exempt from the `compute_terminus_skip_oids` first/last dedup in `07_extract_stops.py`.

The dedup is built on the assumption that a feature whose first stop and last stop share a `stop_id` represents the two directions of the same line meeting at a shared platform, so one of the two dots can be safely dropped. For direction-collapsed modes that pairing cannot happen: two directions of the same variant are already one feature. Any first-stop / last-stop pair at the same `stop_id` therefore belongs to two distinct **variants** of the same ref (different stop sequences → different `merged_stop_set` → different `trip_group_id` → different pfaedle shapes landing on different OSM nodes at the same pier / cable-car station). Each variant's pfaedle-snapped endpoint needs its own dot.

Aerial has an additional reason on top of this: at cascade stations where one section ends and another begins (Stockhornbahn lower → upper aerial at Chrindi, Niederhornbahn funicular → upper aerial at Beatenberg), the GTFS feed assigns a single bare UIC across both sections even though each section is a separate physical aerialway — one dot per (line geometry, stop_id) is required regardless of the directionality argument.

Tram, bus, regional_bus, train, and metro keep per-direction splits, so the dedup's "shared platform" assumption holds for them and they remain subject to it.
