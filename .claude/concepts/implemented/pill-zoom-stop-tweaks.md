...
The pill-appear-zoom and width-base floor mechanism have been revised again since this concept was first written; the current authoritative descriptions live in `pill-rendering.md` § "Dot-to-pill zoom switch" (pills at z14 for every mode) and § "Visual style: dots, discs, pills, connectors" (additive per-zoom `wb_floor(z)`, replacing the per-mode hard clamp described below). The per-mode `stop_width_base_floor` config block and step 07's `_clamp_stop_wb` are removed as part of that revision — the additive per-zoom floor is the only stop-width minimum mechanism. The indicator sections in this doc remain current.
