...
The pill-appear-zoom and stop-sizing mechanism have been revised again since this concept was first written; the current authoritative descriptions live in `pill-rendering.md` § "Dot-to-pill zoom switch" (pills at z14 for every mode) and § "Visual style: dots, discs, pills, connectors" (per-zoom `min_d(z)` / `max_d(z)` range mapping with `min(wb, WB_HIGH)` clamp — replacing the per-mode hard clamp described below AND the earlier additive `wb_floor(z)` mechanism). The per-mode `stop_width_base_floor` config block and step 07's `_clamp_stop_wb` are removed as part of that revision. The indicator sections in this doc remain current.
