...
- Build two lookups from every train-station stop, case-folded: `{full name → [coords]}` (the station's first-comma-segment, "Bern", "Bern Wankdorf", "Zürich HB") and `{city_key → [coords]}` holding both the full name and its space-split first word (catches "Zürich" from "Zürich HB" or "Basel" from "Basel SBB").
- A station named after two towns — exactly two hyphen-joined capitalised parts, "Schaan-Vaduz", "Arth-Goldau", "Dornach-Arlesheim" — puts each half into both lookups as a full name at the station's coordinate, so "Goldau, Post" and "Arth, Post" both strip. Three-part names ("Leysin-Grand-Hôtel", "Lancy-Pont-Rouge") are one place and are not split, since "Grand" or "Pont" would be false city names.
- `city_prefix_aliases` (`scripts/transit/config.yaml`) adds extra city names to both lookups at the coordinates of the stations they stand for. Two gaps it closes: bilingual station names while the bus stops use one language (`Fribourg` → "Fribourg/Freiburg", `Sierre` → "Sierre/Siders", `Murten` → "Murten/Morat"), and cities whose stations are all suffixed so no station carries the plain name (`Zürich` → "Zürich HB", `Basel` → "Basel SBB", `Davos` → "Davos Platz" + "Davos Dorf", likewise Interlaken, Grenchen, Klosters, Worb). It also covers bus-stop prefixes that carry a suffix or abbreviation the station's name lacks: canton codes (`Brienz BE` → "Brienz"), "am See"-style additions (`Uetikon am See` → "Uetikon"), districts (`Aarau Rohr` → "Aarau", which then keeps "Rohr, …" through the shortest-prefix step, whereas `Brienz BE` is stripped whole because "BE" is no district), and abbreviations (`La Chaux-de-F` → "La Chaux-de-Fonds", `Affoltern a.A.` → "Affoltern am Albis"). Not aliased on purpose: "X b. Y" villages ("Fahrni b. Thun") — a different place than Y — and the German twin towns "Rheinfelden (D)" / "Laufenburg (D)". An alias naming a station that doesn't exist prints a warning and is skipped.
- For each dot feature, take the prefix before the first comma:
  1. The whole prefix must match a `city_key` with a coordinate within 25 km; otherwise `display_name = stop_name` (unchanged). This gate is what keeps "Brienz BE, Altersheim" intact — there is no station "Brienz BE", and the plain "Brienz" station elsewhere must not strip it down to "BE, Altersheim".
  2. Then try the prefix's word-prefixes shortest-first ("Bern" before "Bern Wankdorf"). The first one that is a **full station name or alias** with a coordinate within 25 km is what gets stripped: "Bern Wankdorf, Bahnhof" → "Wankdorf, Bahnhof", "Zürich Enge, Bahnhof" → "Enge, Bahnhof", "Fribourg/Freiburg Poya, Bahnhof" → "Poya, Bahnhof". Keeping the district part is the point — a bare "Bahnhof" would be indistinguishable from "Bern, Bahnhof". Only full names qualify in this step; first-word keys would turn "Aesch BL, Dorf" into "BL, Dorf" and "Bad Ragaz, Post" into "Ragaz, Post". The remainder must read as a district: if it starts lowercase ("Uetikon am See"), with a parenthesis ("Les Planches (Aigle)") or is a two-letter canton code ("Brienz BE"), the prefix is the town itself and step 3 strips it whole.
  3. No such word-prefix → the whole prefix goes ("Aesch BL, Dorf" → "Dorf", "Fribourg, Gare" → "Gare" via the alias).
- Stripping uses the pill-arrow's `strip_city_prefix` — matches `city + ","` or `city + " "`, so "Berneck" is not matched by "Bern".
