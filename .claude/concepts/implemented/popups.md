...
- **Route from / to buttons.** The station, pill-arrow and place popups all end in the same row, which sets the routing panel's From / To to this location (`transit-routing.md` § Entry points). The control is a small "Route" text label (anthracite, semibold — text rather than an icon, since an icon beside two icon buttons reads as a third, dead control) followed by a brand-red segmented pill, with no group chrome around them. The pill's two halves are split by a white hairline — button labels in the tooltip only; hover darkens only the hovered half. The halves carry the **dot-and-line glyphs**: from is a dot with a line running to the half's inner edge (`o──`), to is the mirror image (`──o`), and the lines reach the edge without padding so the white hairline joins them into one route line `o──|──o`. The same glyphs (plus the via form `──o──`) run through the map context menu and the map's route pins.
...
While a route is displayed (route-display.md) the map's own stop symbology is hidden, so a click on a station lands on the route overlay's disc or pass-through dot instead. Those open the ordinary **station popup**: the route feature supplies the station key, name and anchor coordinate (the disc the user clicked), and the badges / departures per hour are looked up on the map's stop feature for that station in the loaded stop tiles — which is why route mode hides the stop layers by filter rather than by visibility: a source with no visible layer loses its tiles. Discs, pass-through dots and their name labels all hit. Without such a feature the popup degrades to name + route buttons, as it does for a search hit. Priority: after the map's own stop features (which only hit at z17+ pill-arrows in route mode), before pill-arrows and lines.
