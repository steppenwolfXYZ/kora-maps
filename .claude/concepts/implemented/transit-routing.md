...
1. **Popup buttons** — the station, pill-arrow and place popups all carry the same pair of **Route from here** / **Route to here** buttons. Clicking opens the routing panel (if not already open) with that location set as the corresponding endpoint; if the panel is already open, it overwrites the corresponding endpoint. **Route to here** with an empty From additionally prefills From with `current` (when geolocation is available and not denied), so the route loads immediately; a From the user already set is kept. A station resolves to a `station` endpoint, a POI / address to a `point` endpoint carrying the popup's title as display name. Their look is defined in `popups.md` § Shared conventions.
...
3. **Map context menu** — right-click (desktop) or long-press (touch) on any map location opens a small context menu at the click point (its top-left corner sits on the click and stays square to point at it) with the items **Route from here** and **Route to here**, plus **Route via here** on the bike / walk tabs while another via fits (transit vias are stations, so a map point cannot be one there). From / to selection opens the routing panel (if not already open) with a `point` endpoint at the click coord; if the panel is already open, overwrites the corresponding endpoint. **Route to here** prefills an empty From with `current` the same way the popup button does, so the route loads right away. Via selection appends the point as the last via. Each item carries the dot-and-line glyph in brand red (`o──` / `──o──` / `──o`) — the same iconography as the popup route buttons, the panel's From / Via / To field labels and the map's start / via / goal pins.
...
- **Hops advance by coverage, not by results; the display is settled** (`search-coverage-window.md`). The cascade records the spans it has searched — departures for leave-at hops, arrivals for arrive-by hops — from the interval the fork reports having searched (`koraSearchedFrom` / `koraSearchedTo`, requested window plus MOTIS's own contiguous extension). Only journeys anchored inside that span become candidates; a walk-lighter Pareto point hours out is dropped and returns once coverage reaches it. Whole spans are merged (no merge cap), and a journey is shown only once coverage reaches past where any dominator of it could lie (its arrival + slack on the departure axis; + 3 h under minimize walking), so a later load can never retire what is on screen. Journeys on the far side of the query time never prune near-side ones. Quotas are per side (five per extension), so the shown list is the same whichever path — direct query or earlier/later clicks — reached that coverage, and only ever grows at the extended end.
...
- Production deployment of MOTIS: same-origin nginx proxy at `/routing/` to a docker container on the shared Hetzner CAX11 (2 GB memory cap), serving prebuilt indexes imported on the local Mac and shipped via `scripts/deploy/deploy_motis.sh`. See `deployment.md` § MOTIS deploy. *(Later superseded by `server-side-transit-planning.md`: the browser no longer calls MOTIS at all. The whole search cascade — narrow / wide walking budgets, the time-advance hops, the dominance pruning described under § Ranking — runs inside the app's own `GET /api/plan` endpoint against MOTIS over loopback; the proxy location is gone and one user action is one request.)*
