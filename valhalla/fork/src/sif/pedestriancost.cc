// kora fork: full-file overlay of upstream's src/sif/pedestriancost.cc at
// VALHALLA_REF. One addition — the stroller pedestrian model
// (routing-options.md § Stroller mode): `costing_options.pedestrian.
// kora_stroller = true` prices stairs by altitude instead of leaving them
// at the flat per-flight step_penalty. Everything else is upstream,
// byte for byte; search for "kora fork" to find the delta.
...
// kora fork: stroller stairs model (routing-options.md § Stroller mode).
// Stairs are priced by ALTITUDE, derived from the stairs edge's length at
// a fixed rise per metre: the DEM is far too coarse to measure one
// flight, and OSM rarely tags step_count. Three classes:
//   1-2 steps (length <= kStrollerShortStairsM): a few seconds of cost
//     only — bumping a stroller over a kerb step is nothing.
//   up to kStrollerMediumRiseM of rise: kStrollerMediumSecPerRiseM of
//     REAL elapsed time per metre of rise (carrying), no extra cost.
//   more: kStrollerLongSecPerRiseM elapsed per metre of rise PLUS a
//     search-only penalty of kStrollerLongPenaltySecPerRiseM per metre,
//     so a long flight is taken only when nothing else connects the
//     endpoints — and then at honest time. The elapsed rate is steeper
//     than the medium one on purpose: the transit search compares
//     journeys by duration alone, so a transfer whose only option is a
//     long flight has to look slow, not merely costly.
// These constants and the matrix costing must describe the same walker;
// changing them means rebuilding the stroller footpath matrix.
constexpr float kStrollerStairRisePerMetre = 0.5f;
constexpr float kStrollerShortStairsM = 1.5f;
constexpr float kStrollerShortStairsPenaltySec = 3.0f;
constexpr float kStrollerMediumRiseM = 2.0f;
constexpr float kStrollerMediumSecPerRiseM = 40.0f;
constexpr float kStrollerLongSecPerRiseM = 80.0f;
constexpr float kStrollerLongPenaltySecPerRiseM = 300.0f;
...
public:
  ...
  bool stroller_;                  // kora fork: stroller stairs model on
...
PedestrianCost::PedestrianCost(const Costing& costing)
  ...
  // kora fork: stroller model (foot access rules, stairs priced by rise).
  stroller_ = costing_options.kora_stroller();
...
Cost PedestrianCost::EdgeCost(const baldr::DirectedEdge* edge,
  ...
  // kora fork: stroller stairs model — see the constants block. Elapsed
  // seconds are real (they reach leg durations and the footpath matrix);
  // the long-flight penalty is cost only.
  if (stroller_ && edge->use() == Use::kSteps) {
    const float length_m = static_cast<float>(edge->length());
    if (length_m <= kStrollerShortStairsM) {
      return {sec * factor + kStrollerShortStairsPenaltySec, sec};
    }
    const float rise_m = length_m * kStrollerStairRisePerMetre;
    if (rise_m <= kStrollerMediumRiseM) {
      const float carry = rise_m * kStrollerMediumSecPerRiseM;
      return {sec * factor + carry, sec + carry};
    }
    const float carry = rise_m * kStrollerLongSecPerRiseM;
    const float penalty = rise_m * kStrollerLongPenaltySecPerRiseM;
    return {sec * factor + carry + penalty, sec + carry};
  }
...
void ParsePedestrianCostOptions(const rapidjson::Document& doc,
  ...
  JSON_PBF_RANGED_DEFAULT(co, kMultimodalStartEndMaxDistanceRange, json,
                          ...
                          warnings);
  // kora fork: stroller stairs model (proto field added by
  // patches/options-proto-kora-bicycle.patch).
  JSON_PBF_DEFAULT_V2(co, false, json, "/kora_stroller", kora_stroller);
}
