...
TripLeg_Edge* AddTripEdge(const AttributesController& controller,
  ...
  if (mode == sif::TravelMode::kBicycle) {
    ...
    // kora fork: also when the edge is walkable but not ridable in the
    // traversal direction — the pushed-bike sections the Kora bicycle
    // costing admits (bicycle-costing-fork.md). Maneuvers never merge
    // across a travel-mode change, so pushed sections come back as
    // pedestrian-mode maneuvers the client can draw dotted.
    if (directededge->dismount() || directededge->use() == Use::kSteps ||
        (!(directededge->forwardaccess() & kBicycleAccess) &&
